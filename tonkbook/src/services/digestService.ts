...
export class DigestService {
    ...
    try {
      ...
      for (const query of sortedQueries) {
        ...
        try {
          ...
          totalItemsCollected += results.reduce(
    ...
    const messages = [
      {
        ...
        content:
          "You are an AI assistant that creates concise, insightful summaries of daily news digests. Focus on key themes, important developments, and notable trends across all the collected information.",
      },
      {
        role: "user" as const,
  ...
  async isWebSearchAvailable(): Promise<boolean> {
    try {
      const response = await fetch(
        "http://localhost:6080/api/brave-search/health",
        {
          method: "GET",
        },
      );
